...
public class EsUtil {
    ...
    public String update(String indexName, String id, String jsonStr) {
        ...
        if (StringUtils.isEmpty(searchById)) {
            return null;
        }
     ...
     * 4, 权重计算，权重越高越靠前:  给name精确查询提高权重为2 QueryBuilders.termQuery("name", "kimchy").boost(2.0f)
    ...
    public List<Map<String, Object>> findAll(String indexName, QueryBuilder builder) {
        ...
        return handleResponse(response, 1, 100).getRecords();
    }
     ...
     */
    @SneakyThrows
    public Page<Map<String, Object>> fromSizePage(String indexName, QueryBuilder queryBuilder, BasePageForm basePageForm) {
        ...
        if (StringUtils.isNotBlank(basePageForm.getOrderBy())) {
     ...
     */
    @SneakyThrows
    private Page<Map<String, Object>> handleResponse(SearchResponse response, int pageNum, int pageSize) {
        ...
        return new Page(result, response.getHits().getTotalHits().value, pageSize, pageNum);
    }
     ...
     */
    @SneakyThrows
    public Page<Map<String, Object>> searchAfterPage(String indexName, QueryBuilder queryBuilder, BasePageForm basePageForm) {
        // 构建分页搜寻器
        ...
        sourceBuilder.query(queryBuilder)
                .from(0)
                .size(basePageForm.getPageSize());
        if (org.apache.commons.lang.StringUtils.isNotBlank(basePageForm.getOrderBy())) {
            sourceBuilder.sort(basePageForm.getOrderBy(), basePageForm.getOrderType() ? SortOrder.ASC : SortOrder.DESC);
        }
        ...
        Page<Map<String, Object>> page = handleResponse(response, basePageForm.getPageNum(), basePageForm.getPageSize());
